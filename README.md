...
- **界面语言**: 支持简体中文, 繁体中文和英文; 默认跟随系统语言, 也可在设置页固定.
  切换后界面文案与行情单位立即生效, 例如 `61.1万手` 在英文下显示为 `611.0K lots`.

## 交互方式

- `esc`: 打开菜单; 详情已打开时优先关闭详情, 菜单已打开时关闭菜单.
- 菜单内使用 `↑`/`↓` (或 `j`/`k`), `enter` 或数字键选择页面.
- 看板内使用 `↑`/`↓` (或 `j`/`k`), `enter`, `r`.
- 删除自选股页用 `↑`/`↓`/`←`/`→` (或 `h`/`j`/`k`/`l`) 移动光标, `空格` 勾选, `enter` 提交.
- 设置页使用 `↑`/`↓` (或 `j`/`k`) 选择配置, 使用 `←`/`→` (或 `h`/`l`) 或 `enter` 调整; `d` 恢复默认值.
- 设置页的 `界面语言` 在 `自动` / `简体中文` / `繁體中文` / `English` 之间循环; `自动` 表示跟随系统.
...
`LANG=en_US.UTF-8 leek-box-cli` 会以英文启动; 设置页里选定的语言优先于系统语言,
