...
- **行选择与详情**: `↑`/`↓` (或 `j`/`k`) 选择股票, `enter` 打开基础行情和趋势图; 数字键 `1`-`6` 切换分时, 五日, 日 K, 周 K, 月 K 和年 K.
- **多周期行情**: 分时与五日分钟走势每 30 秒刷新; 日 K, 周 K, 月 K 和年 K 每 5 分钟刷新.
- **刷新控制**: `r` 立即刷新; 在设置命令以 500 ms 步进调整自动刷新间隔, 范围为 1-60 秒.
- **自选股管理**: 支持添加, 删除沪深北 A 股与 ETF; 股票代码可写为 `600000`, `sh600000`, `600000.SH` 等形式.
- **容错展示**: 停牌, 部分行情缺失和刷新失败均有独立状态; 后续轮询成功后自动恢复.
- **界面语言**: 支持简体中文, 繁体中文和英文; 默认跟随系统语言, 也可在设置命令固定.
  切换后界面文案与行情单位立即生效, 例如 `61.1万手` 在英文下显示为 `611.0K lots`.

## 交互方式

- `esc`: 打开菜单; 详情已打开时优先关闭详情, 菜单已打开时关闭菜单.
- 菜单内使用 `↑`/`↓` (或 `j`/`k`), `enter` 或数字键选择命令.
- 看板内使用 `↑`/`↓` (或 `j`/`k`), `enter`, `r`.
- 删除自选股命令用 `↑`/`↓`/`←`/`→` (或 `h`/`j`/`k`/`l`) 移动光标, `空格` 勾选, `enter` 提交.
- 设置命令使用 `↑`/`↓` (或 `j`/`k`) 选择配置, 使用 `←`/`→` (或 `h`/`l`) 或 `enter` 调整; `d` 恢复默认值.
- 设置命令的 `界面语言` 在 `自动` / `简体中文` / `繁體中文` / `English` 之间循环; `自动` 表示跟随系统.
- 股票详情内使用 `1`-`6` 切换 `分时`, `五日`, `日 K`, `周 K`, `月 K`, `年 K`.
- `q`: 没有菜单或详情浮层时退出; 浮层打开时不退出, 避免误触.
- 状态栏右侧显示 Asia/Shanghai 时区的 `YYYY-MM-DD HH:MM:SS`.

语言检测顺序为 `LC_ALL`, `LC_MESSAGES`, `LANG`, 均为 `C`/`POSIX` 或无法识别时再使用 Node 的
`Intl` 系统区域; 仍然无法识别时使用简体中文. 例如 `LANG=zh_TW.UTF-8 leek-box-cli` 会以繁体启动,
`LANG=en_US.UTF-8 leek-box-cli` 会以英文启动; 设置命令里选定的语言优先于系统语言,
`leek-box-cli -h` 的帮助文本也遵循已配置的语言.
...
leek-box-cli settings      # 设置界面语言, 主题与请求参数
leek-box-cli -v            # 查看版本
leek-box-cli -h            # 查看帮助
```
...
2. 选择发布类型: `patch` (0.0.x), `minor` (0.x.0) 或 `major` (x.0.0); 使用 `↑`/`↓` (或 `j`/`k`) + `enter`, 或直接按 `1`/`2`/`3`.
