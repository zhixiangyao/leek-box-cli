...
export const COLUMNS: readonly Column[] = [
  ...
  { kind: 'value', title: '成交量', width: 10, align: 'right', render: (q) => formatVolume(q.volume) },
...
/**
 * 滚动窗口 [start, end): 保证选中行 selected 可见, 不足一屏时全部显示.
 * 表头固定在外层, 滚动只作用于股票行序列.
 */
export const visibleWindow = (total: number, selected: number, visible: number): { start: number; end: number } => {
  if (total <= visible) return { start: 0, end: total }
  const maxStart = total - visible
  const start = Math.min(Math.max(selected - visible + 1, 0), maxStart)
 ...
 * 可视行数 = 终端行数 - 5 (边框 2 + padding 2 + StatusBar 1) - 表头 1 - errorLine 1.
 */
export const tableSlices = (
  ...
  const visibleRows = Math.max(1, rows - 5 - 1 - (errorLine ? 1 : 0))
  const { start, end } = visibleWindow(quotesLength + missingLength, selected, visibleRows)
  return {
